...
2. **Fonts in `src/app/layout.tsx` are in scope, narrowly.** The design uses Space Grotesk (Bold) and Inter (Regular). Neither is loaded today (`layout.tsx` imports `Geist` + `Geist_Mono`). PBI 01 adds `Space_Grotesk` and `Inter` from `next/font/google` to `layout.tsx` as CSS variables (`--font-space-grotesk`, `--font-inter`) alongside the existing Geist variables — the existing Geist variables remain available so any current consumers keep working. This is the only file PBI 01 touches outside `src/app/widgets/pomodoro/`.
...
- **Modified files:**
  - `src/app/layout.tsx` — add `Space_Grotesk` (weight `700`) and `Inter` (weight `400`) imports from `next/font/google`, expose them as `--font-space-grotesk` and `--font-inter` CSS variables on the `<html>` element. Keep the existing `Geist` and `Geist_Mono` variables untouched.
...
- **Visual contract — Figma node `1:3` (literals from intake P4, no Figma variables):**
  - Page background `#030712`; container max width `1280px`, `32px` top padding, `~58.5px` horizontal padding from the app frame edges.
  ...
  - Card: width `~410.66px`, height `~478px`, background `#101828`, `1px` border `#1E2939`, radius `10px`, padding `33px` (with a `1px` bottom adjustment from the design).
